...
try:
    from adaptus.server import (
        mcp, _read_code, _read_resource, _read_template,
        _basic_metrics, _semantic_analysis, _score_formula,
        analyze_debt, score_debt, generate_tests, summarize,
        get_formula, get_guidelines, get_core_prompts
    )
...
class TestResourceFunctions:
    ...
    def test_read_resource_exists(self):
        ...
        assert "Design Guidelines" in result
    ...
    def test_read_template_with_variables(self):
        ...
        with patch('adaptus.server.Path') as mock_path:
            ...
            mock_path.return_value / "core/test.md" = mock_file
            
            result = _read_template("core/test.md", {"name": "Alice", "project": "Adaptus"})
...
""")
    ...
    def test_generate_tests_pytest(self):
        ...
        result = generate_tests("example_module", "pytest")
        ...
        assert "class TestExampleModule" in result["template"]
        assert "import pytest" in result["template"]

    def test_generate_tests_unittest(self):
        """Test generate_tests with unittest framework"""
        result = generate_tests("example_module", "unittest")
        
        assert result["framework"] == "unittest"
        assert "class TestExampleModule(unittest.TestCase)" in result["template"]
...
class TestMcpResources:
    ...
    def test_get_guidelines_resource(self):
        ...
        assert "Design Guidelines" in result
        assert "Python Code Quality Standards" in result
        assert "# Version:" in result
    ...
    def test_get_core_prompts_resource(self):
        ...
        assert "Core Prompt Templates" in result
        assert "Debt Analysis Template" in result
