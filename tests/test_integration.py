...
try:
    from adaptus.server import mcp, analyze_debt, score_debt, generate_tests, summarize
...
""")
        ...
        for file_path, metrics in analysis_result.items():
    ...
    def test_semantic_analysis_integration(self, tmp_path):
        ...
        problematic_code = """
def very_long_function_that_should_be_refactored(param1, param2, param3, param4, param5, param6, param7, param8):
    result = []
...
""")
        ...
        assert "self.assertRaises" in unittest_result["template"]
...
def complex_function(data, config, options, parameters, settings, flags, mode, debug):
    result = []
    for item in data:
        if item.get('active'):
            if config.get('validate'):
                if options.get('strict'):
                    if parameters.get('check_all'):
...
""")
        ...
        complex_hotspot = next(
            (h for h in hotspots if h["path"] == "complex.py"), 
            None
        )
